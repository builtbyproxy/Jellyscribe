...
- [x] 1.1 `LetterboxdSync/SyncHistory.cs`: add `Requested` to the
      `SyncStatus` enum, appended after `Rewatch` so existing numeric
      values (`0`-`3`) are unchanged.
- [x] 1.2 `LetterboxdSync/SyncHistory.cs`: add `SyncEventSources.SeerrAutoRequestFilm`
      and `SyncEventSources.SeerrAutoRequestTv` constants, alongside the
      existing `DiaryImport`.
- [x] 1.3 Test: a round-trip test (record a `Requested`/`Failed` event with
...
- [x] 2.1 `LetterboxdSync/SeerrClient.cs`: extend `GetMovieInfoAsync`'s
      ...
      fetched).
- [x] 2.2 `LetterboxdSync/SeerrClient.cs`: add `GetSeriesInfoAsync`
...
- [x] 2.3 Test: `GetMovieInfoAsync` returns the parsed title on a mocked
...
- [x] 3.1 In the Seerr auto-request loop (`SyncOneUserAsync`), on
...
- [x] 3.2 Same loop, on `RequestResult.Failed` (including the existing
...
- [x] 3.3 Confirm `RequestResult.AlreadyExists` records nothing (no change
      ...
      noise for it).
- [x] 3.4 Test: `WatchlistSyncRunner` unit tests covering all three
...
- [x] 4.1 In `SeerrIntegrationAsync`'s auto-request loop, on
...
- [x] 4.2 Same loop, on `RequestResult.Failed`: record a `SyncEvent` with
      `Status = Failed`, `Source = SeerrAutoRequestTv`, `Error` set from
      the failure.
- [x] 4.3 Confirm `RequestResult.AlreadyExists` records nothing, same as
      3.3.
- [x] 4.4 Test: `SerializdWatchlistSyncRunner` unit tests mirroring 3.4's
      three outcomes, using `SeerrClientFactoryOverride`.

## 5. API surface

- [x] 5.1 `LetterboxdSync/Api/LetterboxdController.cs`: `GetStats` gains a
...
- [x] 5.2 Confirm `GetHistory` needs no shape change (it already returns
      raw `SyncEvent` objects; `Requested` passes through as-is), add a
      test asserting that rather than leaving it assumed.
- [x] 5.3 Test: `LetterboxdControllerTests` (or equivalent) covering
      `GetStats`'s new `requested` field with a mix of statuses recorded.

## 6. Dashboard rendering

- [x] 6.1 `LetterboxdSync/Web/userPage.html`: add `Requested` to the
...
- [x] 6.2 `LetterboxdSync/Web/statsPage.html`: update both inline
