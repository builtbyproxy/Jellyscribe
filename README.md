...
- **Jellyfin 10.11.9 or newer, including Jellyfin 12.x.** One release serves both: verified on 2026-09-16 by loading the shipped v2.4.1 build on a clean Jellyfin 12.0.0 server, where it loads, initialises, and runs its scheduled tasks. There is no separate Jellyfin 12 build to install and nothing to change in your config.
